...
def main():
    ...
    try:
        ...
        kev_raw["known_ransomware_campaign_use"] = (
            kev_raw["known_ransomware_campaign_use"]
        ...
        kev_raw.to_sql("kev_cves", engine, schema="staging", if_exists="append", index=False, method="multi")
        ...
        breaches_stg = breaches[[
            "event_id","event_date","industry","breach_type","records_affected",
            "location","ransomware_flag","mega_breach","source_url"
        ]].copy()

        breaches_stg.to_sql("breach_events", engine, schema="staging", if_exists="append", index=False, method="multi")
        ...
        mart1.to_sql("industry_time_metrics", engine, schema="mart", if_exists="append", index=False, method="multi")

        # --- Mart: KEV pressure (monthly + 30d rolling)
        kev = pd.read_sql("""
            SELECT cve_id, date_added
            FROM staging.kev_cves
        ...
        mart2 = (
            mart2.merge(kev_month, on="period_start", how="left")
                 .merge(kev30, on="period_start", how="left")
                 .merge(cvss_month, on="period_start", how="left")
        )
        ...
        mart2.to_sql("kev_pressure", engine, schema="mart", if_exists="append", index=False, method="multi")

        # --- Mart: model training set
        kev_added_90d = []
        for m in all_months:
            start = pd.Timestamp(m)
        ...
        train = breaches_stg.merge(pressure, on="period_start", how="left")
        ...
        train_out.to_sql("model_training_set", engine, schema="mart", if_exists="append", index=False, method="multi")
